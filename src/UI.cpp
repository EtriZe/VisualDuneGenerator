...
void UI::drawControls(Params& P, int W, int H, UiState& S, const std::string& cfgPath)
{
    ...
    if(ImGui::CollapsingHeader("Bruit principal", ImGuiTreeNodeFlags_DefaultOpen)){
        S.needUpdate |= ImGui::SliderInt ("Octaves",&P.octaves,1,8);
        S.needUpdate |= ImGui::SliderFloat("Freq",&P.freq,0.002f,0.03f);
        S.needUpdate |= ImGui::SliderFloat("Lacunarity",&P.lacunarity,1.0f,3.0f);
        S.needUpdate |= ImGui::SliderFloat("Gain",&P.gain,0.1f,0.9f);
        S.needUpdate |= ImGui::SliderFloat("Amplitude",&P.amp,5.f,100.f);
    }
    ...
    if(ImGui::Button("Regenerate")) S.needUpdate=true;
    ...
    if(ImGui::Button("Save config")){
        if(ConfigKV::save(P, cfgPath)) S.configStatus = "Config sauvegardee: " + cfgPath;
        else S.configStatus = "Erreur sauvegarde config";
    }
    ...
    if(!S.configStatus.empty()) ImGui::TextWrapped("%s", S.configStatus.c_str());
    ...
    float minH, float maxH)
{
    ...
    if(ImGui::Button("Export heightmap (PGM)")) S.requestExportPGM = true;
    ImGui::SameLine();
    if(ImGui::Button("Export heightmap (PNG)")) S.requestExportPNG = true;
    ImGui::SameLine();
    ...
    if(imgH > 0.0f && imgW > 0.0f){
        ...
        if(heightTex != 0) ImGui::Image((ImTextureID)(intptr_t)heightTex, ImVec2(targetW, targetH));

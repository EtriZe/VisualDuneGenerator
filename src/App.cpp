...
int App::run()
{
    ...
    if(state_.autoLoadConfigOnStart){
        if(ConfigKV::load(P_, cfgPath_)) state_.configStatus = "Config chargee au demarrage: " + cfgPath_;
        else state_.configStatus = "Aucune config startup (ok): " + cfgPath_;
    }
    ...
    if(state_.autoSaveConfigOnExit) ConfigKV::save(P_, cfgPath_);
...
void App::handleEvents_()
{
    ...
    while(SDL_PollEvent(&e)){
        ...
        if(e.type==SDL_MOUSEBUTTONDOWN && e.button.button==SDL_BUTTON_RIGHT){
            dragging_=true; lastX_=e.button.x; lastY_=e.button.y;
        }
...
void App::mainLoop_()
{
    while(!quit_){
        ...
        if(doRebuild) rebuildAll_(false);
        else if(doUpdate) updateHeights_();
